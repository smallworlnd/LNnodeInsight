...
Routing fees and selling channels are not the only sources of income possible for routing nodes. Information gathered by routing nodes is also valuable. [LNnodeInsight](https://lnnodeinsight.com) is offering to buy routing node Mission Control data (currently limited to `lnd` nodes, other implementations in the future!). But why MC data? Liquidity tests are inherent to the LN right now. They're needed to be able to maintain a routing node, which means liquidity tests are a form of work, and that Proof-of-Work has value. MC data also offers a good privacy balance. It doesn't reveal forwards, channel balances or payment outcomes.

Here's the current payout framework^[Amounts subject to change]:

- Free accounts can earn up to 100 sats per submission
- Subscribers can earn up to 500 sats (!!!) per submission

So how does it work? First, you'll need to login. Then, head to the Sats4stats tab, and initiate the sale. Simple! Here's a video demonstration:
