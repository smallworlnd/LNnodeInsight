...
dashboardbody <- dashboardBody(
	...
	tabItems(
		...
		tabItem(tabName='chansim',
			fluidRow(column(8,
				box(selectizeInput(inputId="subject", label='Step 1: enter your pubkey or alias', choices=NULL, options=list(placeholder="Pubkey or alias", maxItems=1)), background="yellow", width=NULL),
				box(fluidRow(
						column(10,
							selectizeInput(inputId="target", label='Step 2: enter or select pubkey or alias of a node to simulate add/remove channel', choices=NULL, options=list(placeholder="Pubkey or alias"))),
						column(2,
						...
						column(12,
							sliderInput(inputId='tot.capacity.filt', label='Filter by range of total capacity (in bitcoin)', min=0.1, max=100, step=0.1, value=c(0.1, 100), ticks=FALSE)),
						column(12,
							sliderInput(inputId='avg.capacity.filt', label='Filter by range average channel capacity (in bitcoin)', min=0.01, max=1.4, step=0.01, value=c(0.01, 1.4), ticks=FALSE)),
						column(12,
